...
// Read By ID
// Test URL: http://localhost:8081/employee?id=6983a36e81c7a62628a0b3f1 (updated)
app.get('/employee', async (req, res) => {

  // const employees = await employeeModel.find({_id: req.query.id});          // Returns array of objects
  // const employees = await employeeModel.findById(req.query.id);   // Return a single document/object
  const employees = await employeeModel.find({_id: req.query.id}).select("firstname lastname salary");   // Returns subset
...
app.get('/employees/firstname/:name', async (req, res) => {
  ...
  try {
    if(employees.length != 0) {
      // Using Virtual Field Name
      console.log(`Full name (virtual): ${employees[0].fullname}`)

      // Using Instance method
      console.log(`Full name (method): ${employees[0].getFullName()}`)
